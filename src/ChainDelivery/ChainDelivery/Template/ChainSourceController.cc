...
// $Id: ChainSourceController.cc,v 1.12 2001/05/31 16:55:18 cdj Exp $
//
// Revision history
//
...
ChainSourceController<Binder>::ChainSourceController( 
   ...
   m_havePeekedAtNextSource(false),
...
ChainSourceController<Binder>::synchronizeSource( const SyncValue& iSyncTo )
{
  ...
  if( iSyncTo != currentSyncValue() ){
...
#else
    Stream::Type oRecordTypeOfNextRecord( Stream::kNone );
    ...
    while( kSyncWarningAtEndOfSource == syncStatus && !noMoreSources ){
      ...
      SearchStatus searchStatus = peekAtRecordOfNextSource( 
      						   oSyncValueOfNextRecord,
       						   oRecordTypeOfNextRecord );

      if( kSearchNoError == searchStatus ) {
	if ( oSyncValueOfNextRecord <= iSyncTo ){
	  if( ! switchToNextSource() ){
	    //Had problems
	    return kSyncProblemWithSource;
	  } else {
	    syncStatus = (*m_controller).synchronizeSource( iSyncTo );
	  }
	} else {
				 ...
				 Stream::Type& oRecordTypeOfNextRecord )
{
   ...
   if( kSearchEndOfSource == searchStatus ){
      ...
      searchStatus = peekAtRecordOfNextSource( oSyncValueOfNextRecord,
					       oRecordTypeOfNextRecord );
   }
   return searchStatus;
...
{
   //We are going to the next source, so clear this flag
   m_havePeekedAtNextSource=false;
...
template< class Binder >
DataSourceController::SearchStatus
ChainSourceController<Binder>::peekAtRecordOfNextSource(
   SyncValue& oSyncValueOfNextRecord,
   Stream::Type& oRecordTypeOfNextRecord )
{
   if( ! m_havePeekedAtNextSource ){
      m_havePeekedAtNextSource = true;
      DataSourceController* tempController(0);
      if( 1 < m_binders.size() ){
	 //Temporarily create the next source
	 tempController = m_binders[1].createDataSourceController();
	 ...
	 //Was it initialized alright?
	 if( kInitNoError != (*tempController).initializationStatus() ){
	    m_statusOfNextSource = kSearchProblemWithSource;
	    m_syncValueOfNextSource=SyncValue();
	    m_recordTypeOfNextSource=Stream::kNone;
	 } else {
	    m_statusOfNextSource = (*tempController).findNextRecord(
	       m_syncValueOfNextSource,
	       m_recordTypeOfNextSource );
	 }
      } else {
	 //No more sources
	 m_statusOfNextSource = kSearchEndOfSource;
	 m_syncValueOfNextSource=SyncValue();
	 m_recordTypeOfNextSource=Stream::kNone;
      }
      delete tempController;
   ...
   oSyncValueOfNextRecord=m_syncValueOfNextSource;
   oRecordTypeOfNextRecord = m_recordTypeOfNextSource;
   return m_statusOfNextSource;
}
