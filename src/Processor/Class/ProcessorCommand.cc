...
// $Id: ProcessorCommand.cc,v 1.6 2000/06/01 17:48:09 mkl Exp $
//
...
ProcessorCommand::ProcessorCommand( const Command::Name& name,
   ...
   : LoaderCommand< MasterProcessor >( name, target, false )
{
}
...
ProcessorCommand::execute( int argc, char* argv[] )
{
   ...
   if( 2 <= argc && 0 == strcmp( argv[1], "reorder" ) ) {
	 result = reorderHandler();
      ...
      result = LoaderCommand< MasterProcessor >::execute( argc, argv );
   }
...
ProcessorCommand::helpHandler( )
{
   ...
   result = LoaderCommand< MasterProcessor >::helpHandler();
