...
// $Id: ProducerCommand.cc,v 1.6 2000/06/01 17:48:10 mkl Exp $
//
// Revision history
//
...
ProducerCommand::ProducerCommand( const Command::Name& name,
   ...
   : LoaderCommand< MasterProducer >( name, target, false )
{
}
...
ProducerCommand::execute( int argc, char* argv[] )
{
   ...
   if( 2 <= argc && 0 == strcmp( argv[1], "reorder" ) ) {
	 result = reorderHandler();
      ...
      result = LoaderCommand< MasterProducer >::execute( argc, argv );
   }
...
ProducerCommand::helpHandler( )
{
   ...
   result = LoaderCommand< MasterProducer >::helpHandler();
   
   report( SYSTEM, kFacilityString ) 
      << "\n//  " << name()
      << " reorder <proc1> <proc2> [..] Reorder producers" << endl;
