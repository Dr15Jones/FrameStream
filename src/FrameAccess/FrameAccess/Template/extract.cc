...
// $Id: extract.cc,v 1.12 1999/12/10 14:20:12 cdj Exp $
//
// Revision history
//
// $Log: extract.cc,v $
...
//      = Ifd< DataType >::get( aRecord, key ) ;
   ...
   if( 0 != proxy ) {
      hold = proxy->get( aRecord,
		  RecordMethods< typename FAContainer::contents >::makeKey(
		  UsageTag(aUseLabel), 
		  ProductionTag(aProducerLabel) ) );
   } else {
      throw DANoDataException<typename FAContainer::contents >(
	          aRecord.stream(), 
		  RecordMethods< typename FAContainer::contents >::makeKey(
		         UsageTag(aUseLabel), 
			 ProductionTag(aProducerLabel) ) 
