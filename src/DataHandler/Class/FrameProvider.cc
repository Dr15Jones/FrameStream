...
// $Id: FrameProvider.cc,v 1.6 2000/11/18 19:59:40 cdj Exp $
//
// Revision history
//
...
//
	 ...
	 if ( ! ( aStop.priority() > (*((*provider).second)).stream().priority() ) ) {
	    (*const_cast<FrameProvider*>(this)).m_frame.add( (*((*provider).second)).record( aSyncValue ) ) ;
	 }
      } catch(const DHStreamDidNotSyncException& iException ) {
