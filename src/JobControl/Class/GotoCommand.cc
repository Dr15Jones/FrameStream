...
// $Id: GotoCommand.cc,v 1.22 2000/10/11 19:45:01 mkl Exp $
//
// Revision history
//
...
string( "//                                                                 \n" )+
string( "// Usage:                                                          \n" )+
string( "//  goto <runnumber> <eventnumber>           will proceed to a     \n" )+
...
GotoCommand::execute( int argc, char* argv[] )
{
   ...
   if( 3 == argc ) // assume numerical value
   {
      DABoolean isNumber;
	 ...
	 {
	    DABoolean returnStatus = jobControl->goTo( runNumber, eventNumber );
