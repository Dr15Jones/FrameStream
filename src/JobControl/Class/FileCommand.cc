...
std::string( "// Description: FileCommand                                       \n" )+
std::string( "//                                                                \n" )+
std::string( "// Valid subcommands are:                                         \n" )+
std::string( "//                  (<src>=sourcename, <snk>=sinkname,strm=stream)\n" )+
std::string( "//                                                                \n" )+
std::string( "//  file help                           see this help page        \n" )+
std::string( "//                                                                \n" )+
std::string( "//  file source <src> [<strm1>..]       define src w/ strms       \n" )+
std::string( "//  file src <src> [<strm1>..]          synonym for \"source\"    \n" )+
std::string( "//  file use <src> [<strm1>..]          synonym for \"source\"    \n" )+
std::string( "//  file input <src> [<strm1>..]        synonym for \"source\"    \n" )+
std::string( "//  file in  <src> [<strm1>..]          synonym for \"source\"    \n" )+
std::string( "//                                                                \n" )+
std::string( "//  file add <token> <src> [<strm1>..]  add src to <token>        \n" )+
std::string( "//                                                                \n" )+
std::string( "//  file sink   <snk> <strm1> [strm2..] define sink w/ strms      \n" )+
std::string( "//  file output <snk> <strm1> [strm2..] synonym for \"sink\"      \n" )+
std::string( "//  file out    <snk> <strm1> [strm2..] synonym for \"sink\"      \n" )+
std::string( "//    if format supports specifying data to (not) write           \n" )+
std::string( "//  file sink   <snk> {<strm1> [exclude] { [datatags] } }         \n" )+
std::string( "//    where [datatags] must be enclosed in {} if specifying       \n" )+
std::string( "//    usage and/or production tags                                \n" )+
std::string( "//                                                                \n" )+
std::string( "//  1.) ----------- Sources ----------------                      \n" )+
std::string( "//  To define an input source, use the \"source\" command;        \n" )+
std::string( "//  the system will return a \"token\" which has to be used       \n" )+
std::string( "//  to add new sources to the \"chain\" with name \"token\".      \n" )+
std::string( "//                                                                \n" )+
std::string( "//  You can also create your own token or rename the token        \n" )+
std::string( "//  generated by the system via the \"source\" command;           \n" )+
std::string( "//  Please see the help page for the \"source\" command           \n" )+
std::string( "//  for details.                                                  \n" )+
std::string( "//                                                                \n" )+
std::string( "//  Example:                                                      \n" )+
std::string( "//  i.)  suez> source create myFirstChain                         \n" )+
std::string( "//       suez> file add myFirstChain file1.rp                     \n" )+
std::string( "//       suez> file add myFirstChain file2.rp                     \n" )+
std::string( "//                                                                \n" )+
std::string( "//  OR                                                            \n" )+
std::string( "//  ii.) suez> file src file1.rp                                  \n" )+
std::string( "//        new token \"T_file1\"                                   \n" )+
std::string( "//       suez> source rename T_file1 chain1                       \n" )+
std::string( "//       suez> file add chain1 file2.rp                           \n" )+
std::string( "//       suez> file add chain1 file1.rp                           \n" )+
std::string( "//                                                                \n" )+
std::string( "//  If you don't like the tokens returned by Suez, and you don't  \n" )+
std::string( "//  feel like using example i.) above, you may use the one-liner  \n" )+
std::string( "//                                                                \n" )+
std::string( "//       suez> file add mysource myfile event                     \n" )+
std::string( "//                                                                \n" )+
std::string( "//  which uses the token \"mysource\". (if not used before!)      \n" )+
std::string( "//  (The \"add\" command is normally used for chaining files,     \n" )+
std::string( "//   but the chaining only happens when the second (!) file       \n" )+
std::string( "//   is added, so this trick will always work.)                   \n" )+
std::string( "//                                                                \n" )+
std::string( "//  One more trick: the token returned by Suez is available via   \n" )+
std::string( "//  the command-line interpreter:                                 \n" )+
std::string( "//                                                                \n" )+
std::string( "//       suez> set sourceName [file src file1.rp]                 \n" )+
std::string( "//       suez> echo \"Using $sourceName\"                         \n" )+
std::string( "//                                                                \n" )+
std::string( "//  2.) ------------ Sinks -------------------                    \n" )+
std::string( "//  To define an output sink, use the \"sink\" command.           \n" )+
std::string( "//                                                                \n" )+
std::string( "//  Examples:                                                     \n" )+
std::string( "//       suez> file sink outputFile.pds event                     \n" )+
std::string( "//       suez> file sink outputFile.pds {event {}}                \n" )+
std::string( "//       suez> file sink outputFile.pds {event {DBEventHeader}}   \n" )+
std::string( "//       suez> file sink outputFile.pds {event { {DBEventHeader Mine}}}\n" )+
std::string( "//       suez> file sink outputFile.pds {event { {DBEventHeader {} Mine}}}\n" )+
std::string( "//       suez> file sink outputFile.pds {event exclude {DBEventHeader}}\n" )+
std::string( "//                                                                \n" )+
std::string( "//                                                                \n" )+
std::string( "// Some standard streams are:                                     \n" )+
std::string( "//                        beginrun endrun event                   \n" )+
std::string( "//                        geometry hardware user                  \n" )+
std::string( "//                                                                \n" )+
std::string( "// If no streams are specified, default streams based on          \n" )+
std::string( "// the source format are used.                                    \n" );
...
FileCommand::sourceHandler( )
{
   ...
   else
   {
      ...
      {
	 report( SYSTEM, kFacilityString ) 
	    << "Input Source non-existent: \"" << sourceName << "\"." << std::endl;
	 result = COMMAND_ERROR;
      }
      else 
...
FileCommand::addHandler( )
{
   ...
   else
   {
      ...
      std::string token( getArgument( 2 ) );

      // first the sourceName
      std::string sourceName( getArgument( 3 ) );
...
FileCommand::sinkHandler( )
{
   ...
   {
      // first the sinkName
      std::string sinkName( getArgument( 2 ) );
      ...
      {
	 report( SYSTEM, kFacilityString ) 
	    << "Invalid sinkName: " << sinkName << "." << std::endl;
	 result = COMMAND_ERROR;
      }
      else 
...
//	         ++itStreamData) 
//	    {
//	    	 cout <<"stream "<< (*itStreamData).first.value() <<std::endl;
//	    	 if( (*itStreamData).second.storeThese() ) {
//	    	 	cout <<" store"<<std::endl;
//	    	 } else {
//	    	 	cout <<" do not store"<<std::endl;
//	    	 }
...
{
   static std::string kDoubleQuote("\"");
   std::string missingTags;
   
   std::string tagString( iTagString);
   
   std::string::size_type index = 0;
   std::string::size_type lastNonSpace = tagString.find_last_not_of(kWhiteSpace);
   while( index < lastNonSpace )
   {
     	
   	//see if the datatype has a '{' as part of its definition
   	std::string::size_type braceIndex = tagString.find_first_of("{", index);
   	std::string::size_type nonBraceIndex = tagString.find_first_not_of(" \n\t{",index);
   	
      if( braceIndex < nonBraceIndex ) {
      	//find matching closing brace
      	std::string::size_type closeBraceIndex = tagString.find("}", index);
	std::string::size_type subBraceIndex = tagString.find("{",braceIndex+1);
	...
	do {
	   if( subBraceIndex < closeBraceIndex ) {
		 ...
		 closeBraceIndex = tagString.find("}",closeBraceIndex+1);
      	...
      	if( closeBraceIndex == std::string::npos ) {
      	  ...
      	  report(ERROR, kFacilityString) <<" missing closing brace for \""
					 << std::string( iTagString, braceIndex)
					 << "\""<<std::endl;
      	  throw DAException("missing closing brace");
      	}
      	//look for all three types
      	std::string::size_type firstIndex = tagString.find_first_not_of(kWhiteSpace, 
      	   braceIndex+1);
      	std::string typeTag, usageTag, productionTag;
      	
      	if( closeBraceIndex > firstIndex ) {
      	   std::string::size_type lastIndex = tagString.find_first_of(" \n\t}", firstIndex);
      	   typeTag = std::string(tagString, firstIndex, lastIndex-firstIndex);
		...
		++firstIndex;
      	     ...
      	     if(firstIndex != closeBraceIndex ) {
      	        ...
      	        usageTag = std::string(tagString, firstIndex, lastIndex-firstIndex);
		   ...
		   if ('}' == tagString[lastIndex] ) {
		      ++lastIndex;
      		   ...
      		   if(firstIndex != closeBraceIndex ) {
      		      lastIndex = tagString.find_first_of(" \n\t}", firstIndex );
      		      productionTag = std::string(tagString, firstIndex, lastIndex-firstIndex);
		      //see if there are more items in this list
		      if( lastIndex != closeBraceIndex ) {
			 if( closeBraceIndex != tagString.find_first_not_of(kWhiteSpace, lastIndex+1) ) {
			    throw DAException( (std::string("greater than 3 arguments for data specification: \"")+tagString+"\"").c_str() );
			 }
		      }
      		   }
      		}
      ...
      } else if( nonBraceIndex != std::string::npos ) {
      	std::string::size_type lastIndex = tagString.find_first_of(" \n\t{}",nonBraceIndex);
      	std::string typeTag( tagString, nonBraceIndex, lastIndex-nonBraceIndex );
        ...
        if(typeTag.size() != 0 ) {
            ...
            iTags.insert( 
               DataStringTagsToStore::DatumStringTags(typeTag, std::string(), std::string()) );
         }
         index = lastIndex +1;
      } else {
                                  ...
                                  int index ) const
{
   ...
   while( 0 != ( arg = getArgument( index++) ) )
   {
      ...
      std::string::size_type startOfArg = argument.find_first_not_of(kWhiteSpace);
      if( startOfArg != 0 ) {
	 argument = std::string(arg, startOfArg, argument.size() );
      }
      std::string::size_type endOfStreamPos = argument.find_first_of("{ ");
      std::string::size_type firstBrace = argument.find_first_of("{");
      if( std::string::npos != firstBrace ) 
      {
	...
	count(argument.begin(), argument.end(), '}', nClose);
      	...
      	//find stream
         std::string streamName( arg,startOfArg, endOfStreamPos);
         ...
         std::string::size_type beginOfDataPos = endOfStreamPos;
         //see if we have the qualifier 'exclude'
         DABoolean inclusiveList = true;
         std::string::size_type firstNonSpace = argument.find_first_not_of(kWhiteSpace, endOfStreamPos);
         if( '{' != argument[firstNonSpace] ) {
            const std::string excludeKeyWord("exclude");
            if( excludeKeyWord == argument.substr(firstNonSpace, excludeKeyWord.size() ) ) {
            	inclusiveList = false;
               std::string::size_type endOfQualifier = argument.find_first_of("{",firstNonSpace);
               ...
               report( ERROR, kFacilityString) <<"qualifier \""
                   ...
                   <<"\" not allowed \n  only \"exclude\" is allowed" <<std::endl;
               throw DAException("Bad qualifier");
            }
         }
   	 DataStringTagsToStore::Tags tags;
	 //need to be careful to not get rid of TWO {'s in a row
         std::string::size_type dataStart = argument.find_first_not_of(kWhiteSpace, beginOfDataPos);
	 if( '{' == argument[dataStart] ) {
   	 ...
   	 iMap.insert( std::make_pair( newStreamType, 
