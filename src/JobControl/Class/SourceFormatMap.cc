...
// $Id: SourceFormatMap.cc,v 1.21 2000/12/04 04:43:26 cdj Exp $
//
...
SourceFormatMap::SourceFormatMap( const string& environment )
   : FormatMap< SourceFormatModule >( "SourceFormatMap", environment ),
...
SourceFormatMap::~SourceFormatMap()
{
   ...
   FormatMap< SourceFormatModule >::unloadAll();
...
SourceFormatMap::defaultStreams( const string& iName )
{
   ...
   {
      returnValue 
	 = loadedMap()[ ((*it).second)() ]->defaultStreams();
   }
   else {
      report( ERROR, kFacilityString )
...
void
SourceFormatMap::initialize( const string& iName, SourceFormatModule& iFormat )
{
...
// ------------- overridden Loader< Producer > method
void
SourceFormatMap::finalize( const string& iName, SourceFormatModule& iFormat )
{
		       ...
		       const string& iExtension )
{
   ...
   // first check that iFormatName exists
   _loader_loaded_::iterator which = loaded().begin();
   _loader_loaded_::iterator end = loaded().end();
   while( which != end && !( *which == iFormatName ) ) ++which;
