...
// $Id: JobControl.cc,v 1.104 2001/12/18 21:51:45 cdj Exp $
//
...
JobControl::JobControl()
   : LoaderModule( "JobControl", "JobControl" ),
     ...
     m_sinkM            ( *new SinkManager  ( m_frameDeliverer, 
					      m_frameStorer ) ),
     ...
     m_doDeleteSourceAtEnd( true )
{
   ...
   //addModule( new DBModule( m_sourceM, m_sinkM ) );
...
//      if( true == stop ) break;
	 ...
	 if( !isDone ) 
	 {
	    ...
	    DABoolean shallContinue = processStop( oShallSink );
	 
	    // problem?
	    if( false == shallContinue ) { break; }
	 
	    // only sink in sink mode and iff is to-be-stored stream
	    if( true == oShallSink 
		&& true == frameStorer().contains( stopType ) )
	    {
	       DABoolean sinkStatus = sinkIt();
	       if( false == sinkStatus ) {
		  returnValue = false;
		  break;
	       }
      ...
      {
	 ostream& output = report( ERROR, kFacilityString );
	 output << "-eventLoop: caught a DAException:\n\"" 
		<< thisException.what() << "\"";
	 if( true == m_continueEventLoopAfterException ) {
	    output << "\n; will continue...";
	 }
	 else {
	    output << "\n; will rethrow...";
...
DABoolean
JobControl::goTo( int iRunNumber, int iEventNumber ) 
{
...
DABoolean
JobControl::processStop( DABoolean& oShallSink )
{
   // the returnValue determines if processing continues or not;
   // and oShallSink determines if results get sunk (=written out)
   DABoolean returnValue = true;
   oShallSink = true;

   // get ahold of FrameDeliverer and current Stop
   FrameDeliverer& FD = m_frameDeliverer;
   Stream::Type currentStop = FD.currentStop();

   // check if this stop is valid 
   // (e.g. "false" before we have valid frame, or after end of file)
   if( false == FD.isValidStop() )
   { 
      report( ERROR, kFacilityString ) << "No valid stop!" << endl;
      returnValue = false;
      oShallSink  = false;
   }
   else // stop is valid
   {
      ...
      // execute all actions for that stop
      ActionBase::ActionResult actionResult;
      if( ActionBase::kPassed != 
	  ( actionResult = m_processorM
	                   .executeActions( currentStop, FD.currentFrame() ) )
	 )
      {
	 returnValue = false;
	 oShallSink  = false;
	 ...
	 switch( actionResult )
	 {
	    ...
	    {
	       // issue warning that no action defined for this stop	
	       //report( INFO, kFacilityString )
	       //  << "path execution ended in failed action" << endl;
	       returnValue = true;
	       oShallSink  = false;
	       break;
	    }
	    ...
	    {
	       report( ERROR, kFacilityString )
		  << "path execution ended in Error!" << endl;
	       returnValue = false;
	       oShallSink  = false;
	       break;
	    }
	    case ActionBase::kNoAction:
	    {
		  ...
		  << "path execution: no action for this stop" << endl;
#endif
	       returnValue = true;
	       oShallSink  = true;
	       break;
	    ...
	    case ActionBase::kStopProcessLoop:
	    {
	       // stop "event" loop
	       //report( INFO, kFacilityString )
	       //  << "path execution ended in stop" << endl;
	       returnValue = false;
	       oShallSink  = false;
	       break;
	    }
	    ...
	    {
	       report( ERROR, kFacilityString )
		  << "path execution: unknown error" << endl;
	       returnValue = false;
	       oShallSink  = false;
	       break;
	    }
...
JobControl::sinkIt()
{
   ...
   FrameStorer& FS = m_frameStorer;

   // if we don't have sinks, don't attempt to save frame
   vector<DataSinkDescriptor> sinks = FS.sinks();
   if( true == sinks.empty() ) return( returnValue=true );

   // now try to store frame
   FrameStorer::StoreFrameStatus storeStatus;
  
   Frame& frame = m_frameDeliverer.currentFrame();
   if( FrameStorer::kStoreFrameNoError != 
       ( storeStatus = FS.storeFrame( frame ) ) )
   {
      returnValue = false;
      switch(storeStatus)
      {
	 ...
	 {
	    report( EMERGENCY, kFacilityString ) 
	       << "FrameStorer::kStoreFrameProblemWithASink" << endl;
	    break;
	 }
         case FrameStorer::kStoreFrameUnknownSinkError:
	 {
	    report( EMERGENCY, kFacilityString ) 
	       << "FrameStorer::kStoreFrameUnknownSinkError" << endl;
	    break;
	 }
         case FrameStorer::kStoreFrameUnknownRecordError:
	 {
	    report( EMERGENCY, kFacilityString ) 
	       << "FrameStorer::kStoreFrameUnknownRecordError" << endl;
	    break;
	 }
	 ...
	 {
	    report( EMERGENCY, kFacilityString )
	       << "Unknown Store error from FrameStorer" << endl;
	    break;
	 }
      }
   }
...
FrameStorer&
JobControl::frameStorer()
{
   return m_frameStorer;
}
