...
// $Id: JobControl.h,v 1.46 2001/12/18 21:51:47 cdj Exp $
//
...
class JobControl : public LoaderModule
{
   ...
   public:
      ...
      DABoolean goTo( int runNumber, int eventNumber );
      DABoolean gotoNextStop( Stream::Type& oCurrentStop, DABoolean& oNoError );
      DABoolean processStop( DABoolean& oShallSink );
      DABoolean sinkIt( void );

      // access methods
      FrameDeliverer& frameDeliverer( void );
      FrameStorer& frameStorer( void );
      ...
      const FrameStorer& frameStorer( void ) const;
