...
// Module:      SinkManager
// 
// Description: handles Sinks (internally) and hands off DataSinkBinders to 
//              FrameStorer
//
...
// $Id: SinkManager.h,v 1.10 1999/04/14 21:16:29 mkl Exp $
//
// Revision history
//
// $Log: SinkManager.h,v $
...
class SinkManager : public Module
{
   ...
   public:
      ...
      SinkManager( FrameDeliverer& iDeliverer,
		   FrameStorer&    iStorer );
      virtual ~SinkManager();
      ...
      // remove sinks
      DABoolean editSink( const SinkName& iName );
   ...
   private:
...
#if 0 /* not used anymore: streams of sinks are activated automatically */
      DABoolean checkIfStreamsOfSinkAreActive( 
	 const DataSinkBinder& iBinder ) const;
      ...
      // need to have access to framedeliverer/storer
      FrameDeliverer& m_frameDeliverer;
      FrameStorer&    m_frameStorer;
