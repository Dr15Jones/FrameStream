...
// $Id: TclInterpreter.cc,v 1.37 2002/02/20 16:47:11 cdj Exp $
//
...
Tk_AppInit( Tcl_Interp* interp )
{
   ...
   // tk initialization
   if( TCL_ERROR == ( status = Tk_Init(interp) ) )
   {
      report( SYSTEM, kFacilityString ) 
	 << "unsuccessful Tk initialization" << endl;
      return status;
   }

   // startup gui
   Tcl_Eval(interp, 
    "if [ tcl_file exists /home/mkl/work/cleo3/cvssrc/Suez/gui.tcl] \
    {tcl_source /home/mkl/work/cleo3/cvssrc/Suez/gui.tcl} \
				   ...
				   Tcl_DString& ioCommands)
{
   ...
   if( 0 != iCommandLine && 0 < strlen( iCommandLine ) ) {
      Tcl_DStringAppend( &ioCommands, iCommandLine, -1 );

      if ( ! Tcl_CommandComplete( Tcl_DStringValue( &ioCommands ) ) ) {
...
TclInterpreter::saveHistory()
{
   ...
   Readline::write_history( m_historyFile.c_str() );
   Readline::history_truncate_file( m_historyFile.c_str(), m_historyLength );
}
			       ...
			       int argc, char* argv[] )
{
   ...
   return reinterpret_cast<TclInterpreter*>(Interpreter::interpreter())->sourceCommand(filename);
}
