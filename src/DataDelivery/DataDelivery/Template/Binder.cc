...
#include "DataDelivery/Binder_makeChain.h"
#include "DataDelivery/Binder_makeChainNotChainable.h"
...
//
static const char* const kBinderFacilityString 
= "JobControl.Binder";
...
Binder<DataBinder>::addBinder( BinderBase* iBinder )
{
   ...
   if( type() != iBinder->type() ) {
	 ...
	 << "can't combine binders of different types!" << std::endl;
   ...
   if( true == isChain() ) {
	 ...
	 report( ERROR, kBinderFacilityString )
	    << "can't chain sources of this type" << std::endl;
	 returnValue = false;
      }
   }
...
template< class DataBinder >
DataSourceBinder*
Binder<DataBinder>::makeChain( const std::string& iName,
