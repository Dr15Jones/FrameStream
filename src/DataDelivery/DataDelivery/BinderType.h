...
#define _bindertype_arestandard_ std::vector< DABoolean >
#endif /* _streamtype_priorities_ */


class BinderType : public UTIdentifier< std::string, BinderBase >
{
   ...
   public:
      ...
      BinderType( void );
      BinderType( const std::string& iString,
...
std::ostream& operator<<(std::ostream& iOStream, const BinderType& iBinderType );
std::istream& operator>>( std::istream& iIStream, BinderType& iBinderType ) ;
...
#endif
template<>
class less< BinderType > : public UTIdentifierLess< std::string, BinderBase >
{
};
#if !defined( NO_STD_NAMESPACE_BUG)
}
